...
slug: /platforms/aws-nitro-threats
---
...
<PageBanner
  title=""
  image="/img/banners/PG-04D_Everything_That_Can_Go_Wrong.png"
/>
...
1. **Attestation forgery by compromised Nitro hypervisor or signing key**
    ...
    - Mitigations: Verify attestation chains outside provider trust: require offline root CA you control, use multi-party attestation checks, restrict KMS policies to only the exact PCR0/PCR8 hash you expect (and monitor). ([Reference](https://docs.aws.amazon.com/enclaves/latest/user/set-up-attestation.html))
...
9. **Remote verifier trusting wrong fields / broken verification logic**
    ...
    - Mitigations: Implement strict verification logic (compare exact PCR0/PCR8, check nonce/timestamp, verify signature chain to a trusted root).
...
- **Reproducible builds & provenance**
    - Reproducible builds so you can publish expected PCR0 and PCR8 values; keep deterministic build artifacts. Use SBOM and signed build artifacts.
...
- **Attestation verification logs**: log each attestation (nonce, PCR0, signature), compare to expected list. Flag any new PCR0.
...
1. Lock down KMS key policy to exact PCR0 and require nonce-based challenge. ([Reference](https://docs.aws.amazon.com/enclaves/latest/user/set-up-attestation.html))

2. Minimize enclave surface and harden message parsing code.

3. Implement reproducible builds and publish expected PCR0/PCR8.
