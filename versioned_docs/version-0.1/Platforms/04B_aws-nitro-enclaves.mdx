...
slug: /platforms/aws-nitro-enclaves
---
...
<PageBanner
  title=""
  image="/img/banners/PG-04B_The_Sealed_Room.png"
/>
...
>
  <img
    src="/img/diagrams/nitro-enclaves/mgmt-flowchart.png"
    alt="Nitro Enclave Application Management Flowchart  Diagram"
...
Here is the general format of an Attestation Document:
...
AttestationDocument = {
    ...
    ? public_key: user_data,       ; an optional DER-encoded key the attestation
                                   ; consumer can use to encrypt data with. This key has
                                   ; to be generated by the user-application and must be
                                   ; included in the attestation request
...
The enclave and service which want to attest the enclave agree on a protocol to follow which can be customised using optional parameters in the Attestation Document (`public_key`, `user_data`, `nonce`). Services which rely on enclave security defines this protocol and the enclace software follows it.

The enclave requests an attestation document from the Nitro Attestation Service. This document includes:
...
- An optional nonce provided by the enclave to prevent replay attacks.

- Optional additional user-defined data which can optionally be encrypted using the public key of attestaion consumer.

> Using `nonce` and a `public_key` is highly encouraged to add encryption to user_data in transit and guarantee that the attesation consumer service is interacting with a live enclave (rather than a malicious actor using a previously valid but stale attesation document). 

The document is signed by AWS's Nitro Attestation Public Key Infrastructure (PKI), allowing external services like AWS Key Management Service (KMS) or custom certificate authorities to validate the enclave's identity by communicating over secure TLS channels with KMS. This allows you to create access rules within the KMS so only enclaves with a particular hash (i.e. a specific pre-agreed docker container having the PCR values, running pre-agreed code) get to decrypt/process sensitive data.
...
> NOTE: PCR3 and PCR4 are runtime values generated when the enclave starts and are not part of EIF. These values are cryptographically derived from metadata associated with the EIF and launch context. While PCR3 and PCR4 are not file contents, they are part of the enclave’s measurement chain initiated by the EIF.
...
>
  <img
    src="/img/diagrams/nitro-enclaves/external-enclave.png"
    alt="External to enclave communication Diagram"
    style={{ maxWidth: "90%", height: "auto" }}
  />
</div>
...
>
  <img
    src="/img/diagrams/nitro-enclaves/enclave-external.png"
    alt="Enclave to external communication Diagram"
    style={{ maxWidth: "90%", height: "auto" }}
  />
</div>
...
>
  <img
    src="/img/diagrams/nitro-enclaves/kms-integration.png"
    alt="kms-integration Diagram"
    style={{ maxWidth: "90%", height: "auto" }}
  />
</div>
...
From an implementation standpoint, the parent instance is a blind transport for enclave–to-KMS conversations. It can route traffic, but it does not gain access to plaintext keys. Since enclaves have no network stack, enclave-to-KMS traffic is mediated via the KMS proxy running on the parent instance:
...
> Since KMS Proxy is an untrusted service, it could drop/delay requests or attempt replay attacks if `nonces` are not used in the attestation document.
...
- PCR0: Measures the Enclave Image File (EIF)
...
- Enclaves booted in debug mode generate attestation documents with PCRs that are made up entirely of zeros. These attestation documents can't be used for cryptographic attestation. It is imperative that any production deployment of Nitro Enclaves is not using `--debug-mode` or `--attach-console` launch parameters.
